...
while read -d $'\0' MAILFILE; do 
	...
	if [[ $MOVEM == YES ]]; then
		CMD="mv"
        	IN='/Maildir/*'
        	OUT="/Maildir/$MOVETO/cur"
        	MYMOVETO="${MAILFILE/$IN/$OUT}"
		if [ ! -d "${MYMOVETO}" ]; then
			mkdir -p "${MYMOVETO}"
			mkdir -p "${MYMOVETO/cur/new}"
			mkdir -p "${MYMOVETO/cur/tmp}"
			chown -R $(stat -c "%U:%G" "$MAILFILE") "${MYMOVETO}"
                 	chown -R $(stat -c "%U:%G" "$MAILFILE") "${MYMOVETO/cur/new}"
                	chown -R $(stat -c "%U:%G" "$MAILFILE") "${MYMOVETO/cur/tmp}"
		fi
	else
		CMD="echo mv"
	fi
